...
#########################################################    
# check "and" "or" keywords in user query
def checkAndOr(k):
    if k.strip() == AND:
...
def groupAttributesInSearchGroup(statlist, attcoll):
    ...
    for slist in statlist:
        if getAttributeType(slist[0].strip(), attcoll) == METAATT:
            #print "metadata attribute"
            #add to metadata dict
            addToDict(metadict,slist)

        elif getAttributeType(slist[0].strip(), attcoll) == IMAGEATT:
...
def search(coll, dict):
    ...
    if h5s.count() == 0:
        ...
        print " ",dict
    #else:
        #print "Number of matches for -- " ,dict, " --  : ", h5s.count()
        #for h5 in h5s:
        #    print "group: ",h5["group"]," subgroup: ",h5["subgroup"]," item: ",h5["itemname"]
            #print "-- ", h5
    return h5s


#########################################################                                                                 
#parse user input to create structures needed for db query
